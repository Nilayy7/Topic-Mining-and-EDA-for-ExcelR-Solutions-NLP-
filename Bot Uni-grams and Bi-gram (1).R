...
bot <- data.frame(Bot_Chat = chat_transcript[!grepl("Visitor (.*)", chat_transcript$Chat),]) # separate bot chat

#library(tidyr)
Clean_Chat<- bot %>% separate(Bot_Chat, c("A", "B"),": ",fill = "left")
...
makeposwordc = function(x){
  ...
  windows()
  wordcloud(names,freq_pos,scale=c(5,2),colors = brewer.pal(8,"Dark2"))
}
...
makenegwordc = function(x){	
  ...
  windows()
  wordcloud(names,freq_neg,scale=c(5,3),colors = brewer.pal(8,"Dark2"))
}
...
windows()
wordcloud(sort_two$bitoken,sort_two$Freq,random.order=FALSE,scale = c(4,1),colors = brewer.pal(8,"Dark2"),max.words=150)
