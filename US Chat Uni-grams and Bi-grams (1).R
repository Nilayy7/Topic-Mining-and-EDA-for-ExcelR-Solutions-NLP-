...
US_Visttor_Name <- vector()

for (i in 1:127824){
if (structured_data$Country_Code[i] == " US"){
  
  US_Visttor_Name[i] <-  structured_data$Visitor_Name[i]
}}

US_Visttor_Name <- US_Visttor_Name[!is.na(US_Visttor_Name)]
...
US_Visitor_Chat <- data.frame()

library(doSNOW)
cl <- makeCluster(8, type="SOCK")
registerDoSNOW (cl)
for (i in 1:length(US_Visttor_Name)){
visitor <- data.frame(Visitors_Chat = grep(chat_transcript$Chat,pattern = US_Visttor_Name[i],value = T))
US_Visitor_Chat <- rbind(US_Visitor_Chat,visitor)
}

#write.csv(US_Visitor_Chat,"D:/Data Science Project/Conversation Mining/US_Visitor_Chat.csv", row.names = FALSE)

library(tidyr)
Clean_Chat<- US_Visitor_Chat %>% separate(Visitors_Chat, c("A", "B"),": ",fill = "left")
...
makewordc = function(x){	
  ...
  windows()
  wordcloud(freq.df$word[1:100], freq.df$freq[1:100],scale = c(4,1.5),random.order = F, colors=1:10)
} 
