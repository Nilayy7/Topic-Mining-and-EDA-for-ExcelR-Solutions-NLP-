####################################################### LOading Given Data Sets and Creating Corpus ######################################################################

chat_transcript <- read.csv("D:/Data Science Project/Conversation Mining/chat_transcript.csv")
colnames(chat_transcript) <- c("V1")

chat_transcript$V1 <- iconv(chat_transcript$V1, from = 'UTF-8', to = 'ASCII//TRANSLIT')

df8 <- chat_transcript
df8 <- data.frame(df8[!grepl("================================================================================", df8$V1),])
colnames(df8) <- c("V1")
...
df8 <-  data.frame(df8[!grepl("Hi Welcome to ExcelR Solutions. How can i assist you?", df8$V1),])
colnames(df8) <- c("V1")

library(tm)
df8 <- stemDocument(df8$V1)
corpus <- Corpus(VectorSource(df8))
corpus <- Corpus(VectorSource(df8))
...
corpus_clean = tm_map(corpus_clean, removeWords,c(stopwords("english"),stopwdrds,"ananya","mounica","patel","visitor"))
...
wordcloud(sort_two$bitoken,sort_two$Freq,random.order=FALSE,scale = c(4,0.5),colors = brewer.pal(8,"Dark2"),max.words=250)
